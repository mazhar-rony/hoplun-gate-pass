            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Building</strong>
                        <a href="{{ route('buildings.create') }}" class="btn btn-primary float-right">Add Building</a>
                    </div>
                    <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($buildings as $building)
                                    <tr>
                                        ...
                                        <td>{{ $building->name }}</td>
                                        <td>{!! $building->is_active ? '<span class="badge badge-success">Active</span>' : '<span class="badge badge-danger">Inactive</span>' !!}</td>
                                        <td>
                                            {{-- <a href="{{ route('buildings.show', $building->id) }}" class="btn btn-info btn-sm">View</a> --}}
                                            <a href="{{ route('buildings.edit', $building->id) }}" class="btn btn-primary btn-sm">Edit</a>
                                            <form action="{{ route('buildings.destroy', $building->id) }}" method="POST" onsubmit="return confirmDelete();" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
